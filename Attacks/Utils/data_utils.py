...
def shadow_split_whitebox(graph, ratio, history=None, exist=False, diag=False):
    ...
    if diag:
        ...
        rprint(f"Uniqueness of id_intr: {shadow_graph.ndata['id_intr'].unique(return_counts=True)}")
        rprint(f"Uniqueness of id_inte: {shadow_graph.ndata['id_inte'].unique(return_counts=True)}")
