...
def preprocessing_graph(graph, data_name, n_neighbor, n_layers):
    ...
    sh_nodes = sampling_shadow_nodes_by_label(graph=graph)
    rprint(f"Done spliting remain and shadow. Shadow graph has: {sh_nodes.size(dim=0)}")
...
def generate_attack_samples(graph, conf, conf_nohop, num_class, mode, device):
    ...
    if mode != 'target':
        ...
        samples = torch.cat((conf, conf_nohop, labels), dim=1)
        ...
        # pos_entropy = get_entropy(pred=pos_samples).mean()
        # neg_entropy = get_entropy(pred=neg_samples).mean()
