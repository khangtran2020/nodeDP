...
def run(args, current_date, device):
    ...
    with timeit(logger=logger, task='preparing-attack-data'): 
        ...
        with torch.no_grad():
            ...
            # shadow_conf = torch.cat((shadow_conf, shadow_conf_nohop), dim=1)
            # remain_conf = torch.cat((remain_conf, remain_conf_nohop), dim=1)

            x, y = generate_attack_samples(graph=shadow_graph, conf=shadow_conf, conf_nohop=shadow_conf_nohop,
                                           mode='shadow', num_class=args.num_class, device=device)
            x_test, y_test = generate_attack_samples(graph=remain_graph, conf=remain_conf, conf_nohop=remain_conf_nohop,
                                                     num_class=args.num_class, mode='target', device=device)
